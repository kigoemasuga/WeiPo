	...
	<div class="WP_frame">
		...
		<div class="col-xs-8 col-md-8">
			<ul class="nav nav-pills">
				<li role="presentation"><a href="nodify.do?type=comment">评论</a></li>
				<li role="presentation" class="active"><a
					href="nodify.do?type=favor">赞我</a></li>
			</ul>
			<!-- 未读  -->
			<c:forEach var="favor" items="${requestScope.favors1}">
				<div class="row" data-toggle="modal" data-target="#myWeipoModal"
					onclick="loadXMLDoc('${favor.key.userIDOfBlah}','${favor.key.dateOfBlah.time}');loadComment('${favor.key.userIDOfBlah}','${favor.key.dateOfBlah.time}');
																loadCommentEdit('${sessionScope.login}','${favor.key.userIDOfBlah}','${favor.key.dateOfBlah.time}');readFavor('${favor.key.id}','${favor.key.userIDOfBlah}','${favor.key.dateOfBlah.time}');">
					<div class="row favor" style="background: #FFFFFF">
						<div class="col-xs-2">
							<img src="${favor.value.avatarPath}" width="120px"
								class="img-circle center-block img-responsive img-thumbnail">
						</div>
						<div class="col-xs-3">
							<h2>${favor.value.name}</h2>
							<font style="color: #000FF5">@${favor.value.id}</font>
						</div>
						<div class="col-xs-6 favortxt">
							<p>在&nbsp;&nbsp;${favor.key.dateOfBlah }&nbsp;&nbsp;赞了你</p>
						</div>
					</div>
				</div>
			...
			<hr>
			<!-- 已读  -->
			<c:forEach items="${requestScope.favors2}" var="favor">
				<div class="row" data-toggle="modal" data-target="#myWeipoModal"
					onclick="loadXMLDoc('${favor.key.userIDOfBlah}','${favor.key.dateOfBlah.time}');loadComment('${favor.key.userIDOfBlah}','${favor.key.dateOfBlah.time}');
																loadCommentEdit('${sessionScope.login}','${favor.key.userIDOfBlah}','${favor.key.dateOfBlah.time}');">
					<div class="row favor" style="background: #FFFFFF">
						<div class="col-xs-2">
							<img src="${favor.value.avatarPath}" width="120px"
								class="img-circle center-block img-responsive img-thumbnail">
		...
		<script>
			function readFavor(favourID, userIDOfBlah, dateOfBlah) {
				var xmlhttp;
				...
				xmlhttp.open("GET", "ajax.do?type=readFavor&id=" + favourID
						+ "&userIDOfBlah=" + userIDOfBlah + "&dateOfBlah="
						+ dateOfBlah, true);
				xmlhttp.send();
			}
